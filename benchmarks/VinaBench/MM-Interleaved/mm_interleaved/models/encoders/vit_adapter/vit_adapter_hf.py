...
class CLIPVisionTransformerAdapter(nn.Module):
    ...
    def interpolate_pos_embed(self, image_size, hw_ratio=1.0):
        self.embeddings.interpolate_pos_embed(image_size, hw_ratio=hw_ratio)
    ...
    def forward(
        ...
        pixel_values_resized = F.interpolate(pixel_values, size=(H*16, W*16), mode="bilinear", align_corners=False)
        ...
        c4 = c4.transpose(1, 2).view(bs, dim, H_half, W_half).contiguous()
        ...
        x4 = F.interpolate(x4, size=(H_half, W_half), mode='bilinear', align_corners=False)
...
class CLIPVisionAdapterModel(CLIPPreTrainedModel):
    ...
    def interpolate_pos_embed(self, image_size, hw_ratio=1.0):
        self.vision_model.interpolate_pos_embed(image_size, hw_ratio=hw_ratio)
