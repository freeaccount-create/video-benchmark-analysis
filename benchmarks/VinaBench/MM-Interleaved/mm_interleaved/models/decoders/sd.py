...
class StableDiffusion(nn.Module):
    ...
    def get_negative_prompt_embeds(
        ...
        '''
        self.pipeline = self.pipeline.to(device, dtype)
        ...
        negative_prompt_embeds = self.pipeline.text_encoder(
    ...
    def generate_images(
        ...
        if not gen_h:
            gen_h = self.image_size
        if not gen_w:
            gen_w = self.image_size
        '''
        pipeline = DiffusionPipeline.from_pretrained(
            self.pretrained_model_name_or_path,
            unet=self.unet,
        ...
        self.pipeline = self.pipeline.to(text_embeds.device, text_embeds.dtype)
        self.pipeline.vae = self.pipeline.vae.float()
        images = []
        for num in range(num_validation_images):
            ...
            for mini_iter in range(math.ceil(text_embeds.shape[0] / mini_bs)):
                ...
                latents = 1 / self.pipeline.vae.config.scaling_factor * image
    ...
    def forward(
        ...
        h, w = image.shape[-2:]
        assert not self.vae.use_tiling
        '''
        assert (
