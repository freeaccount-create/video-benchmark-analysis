...
class LMMTrainer(transformers.Trainer):
            ...
            if target_image_idxs_per_sample is not None:
                ...
                for target_image_idx in target_image_idxs_per_sample:
                    ...
                    for image in pil_images:
                        ...
                        if isinstance(image_tensor_pred, tuple):
                            image_tensor_pred = image_tensor_pred[0]  # use the input format
                    ...
                    for sample_id, (image, meta) in enumerate(
                        ...
                        if image_id < num_samples:
                            image_path = os.path.join(
                                save_path, f"{meta[0]}_{target_image_idx:02d}.jpg"
                            )
                            ...
                            if save_gt_image_online:
                                ...
                                image_gt_path = os.path.join(
                                    save_path_gt,
                                    f"{meta[0]}_{target_image_idx:02d}.jpg",
                                )
    ...
    def _inner_generation_loop_v3(
        ...
        **kwargs,
        ...
        generation_kwargs = getattr(
            dataloader.dataset, "generation_kwargs", None
        )
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if target_image_idxs_per_sample == "all":
                assert save_gt_image_online
                assert batch_size == 1
                num_image = inputs["num_image_per_seq"][0].to(int)
                # for target_image_idx in range(1, num_image):
                for target_image_idx in range(num_image):
                    target_image_idxs = torch.zeros_like(inputs["target_image_idxs"]) + target_image_idx
                    inputs["target_image_idxs"] = target_image_idxs
                    print(f"{target_image_idx=} {target_image_idxs=}")

                    image_tensors_dec = inputs["image_tensors_dec"]
                    C, H, W = image_tensors_dec[0].shape
                    inputs["gen_h"] = H
                    inputs["gen_w"] = W
                    ...
                    for image in pil_images:
                        image_tensor_pred = dataloader.dataset.transform(image)
                        if isinstance(image_tensor_pred, tuple):  # for dual transform
                            image_tensor_pred = image_tensor_pred[0]
                        if isinstance(image_tensor_pred, np.ndarray):
                            image_tensor_pred = torch.from_numpy(image_tensor_pred)
                        images_tensors_all.append(image_tensor_pred)
                    image_tensors_pred = torch.stack(images_tensors_all, dim=0)
                    ...
                    for sample_id, meta in enumerate(inputs["meta"]):
                        image_id = (
                            (step * batch_size + sample_id)
                            * self.accelerator.num_processes
                            + self.accelerator.process_index
                        ...
                        if image_id < num_samples:
                            ...
                            image_gen = pil_images[sample_id]
                            image_gen.save(image_path)
    ...
    def evaluation_loop(
        ...
        if dataset_cls_name in ["VISTDataset", "PororoDataset", "FlintStonesDataset", "VWPDataset", "StorySalonDataset"]:
            ...
            else:  # "VWPDataset", "StorySalonDataset"
                eval_outputs = self._inner_generation_loop_v3(
                    dataloader,
                    model,
                    generate_mode=generate_mode,
                    metric_key_prefix=metric_key_prefix,
