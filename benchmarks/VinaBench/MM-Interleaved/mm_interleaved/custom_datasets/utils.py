...
from .vwp import VWPDataset
from .storysalon import StorySalonDataset
...
def _build_dataset(config):
    ...
    elif config.name == "storysalon":
        dataset = StorySalonDataset(
            annt_root=config.annt_root,
            data_root=config.data_root,
            transform=transform,
            phase=config.phase,
...
def create_transform(
    ...
    # hw_ratio: 1.0 (StorySalon) or 0.4375 (VWP and Storyboard20K)
    if aug_type == "numpy":
        assert resize
        ...
        assert resize
        transform = transform_pil(resolution=resolution, hw_ratio=hw_ratio)
    elif aug_type == "flip":
...
class transform_numpy:
    ...
    def __call__(self, pil_image):
        if self.random_crop:
            ...
            else:  # for resize, input size should be (width, height)
                arr = np.array(
                    pil_image.resize(
                        (self.resolution, int(self.resolution*self.hw_ratio)), resample=Image.BICUBIC
                    )
                )
...
class transform_pil:
    ...
    def __call__(self, pil_image):
        pil_image.resize((self.resolution, int(self.resolution*self.hw_ratio)), resample=Image.BICUBIC)
        return pil_image

    def __repr__(self):
        return (
            f"transform_pil: {self.resolution=}"
        )
