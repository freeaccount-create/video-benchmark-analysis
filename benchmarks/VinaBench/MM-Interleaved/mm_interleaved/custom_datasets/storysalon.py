...
class StorySalonDataset(BaseDataset):
    def __init__(
        self,
        data_root,
        annt_root,
    ...
    def load_database(self):
        ...
        with open(self.annt_root, "r") as f:
            ...
            if self.start is not None and self.end is not None:
                salon_data = salon_data[self.start:self.end]
        ...
        for sample in salon_data:
            self.annt_ids.append(sample["portion"]+"_"+sample["sid"])
            self.narratives.append(sample["narrative"])
            self.images.append(sample["image_paths"])
            self.profiles.append(sample["global_profile"])
            ...
            # if link_type == "cap":
            #     links_set.append({})
            for raw_link in sample["key"]["links_to_nar"]:
                links = {}
            ...
            self.key_nar_links.append(links_set)
    ...
    def __repr__(self) -> str:
        return (
            f"StorySalon Dataset phase={self.phase}\n"
    ...
    def _get_caption(self, sid, add_links=False):
        # TO DO: add links
        if add_links:
            key_nar_links = self.key_nar_links[sid]
    ...
    def _get_image(self, img_link, transfrom=True):
        ...
        img_file = os.path.join(self.data_root, "/".join(img_link.split("/")[2:]))
    ...
    def __getitem__(self, index):
        ...
        if "setups" in self.out_mode:
            ...
            if len(sub_text_token) > 25:
                sub_text_token = sub_text_token[:25]
        ...
        for i in range(len(image_links)):
            ...
            if "captions" in self.out_mode:
                ...
                sub_text_token = sub_text.split(" ")
                if len(sub_text_token) > 60:
                    sub_text_token = sub_text_token[:60]
                text += " ".join(sub_text_token)
                ...
                if "setups" in self.out_mode:
                    ...
                    # setup = setup.replace("\nTime: ", " [Time] ")
                    # setup = setup.replace("\nLocation: ", " [Location] ")
                    setup_token = setup.split(" ")
                    if len(setup_token) > 10:
                        setup_token = setup_token[:10]
                    setup = " ".join(setup_token)
                    text += f" {setup} "
            else:
                sub_text_token = sub_text.split(" ")
