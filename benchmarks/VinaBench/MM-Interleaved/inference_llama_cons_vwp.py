...
def add_links(caption, raw_link):
    links = {}
    ...
    idx = 0
    cap_ls = word_tokenize(caption)

    for cap_ent, nar_ent in links.items():
    ...
    caption_with_link = " ".join(cap_ls)
    return caption_with_link


def get_image(data_root, img_link, transform=None):
    out_pth = os.path.join(data_root, img_link.split("/")[-2])
...
def load_annt_data(
    ...
    for info in infos:
        ...
        if start_idx == 0:
            ...
            cap_token = cap.split(" ")
            if len(cap_token) > 100:
                cap_token = cap_token[:100]
            text += " ".join(cap_token) + setup

            text += f"Image {str(start_idx)}: {image_subseq} "
        
        else:
            for idx in range(start_idx):
                scene_id = info["scene_full_id"]
                story_id = str(info["story_id"])
                
                img_in, img_gold = get_image(data_root, info["image_links"][idx], transform)
...
def inference_all(model, config, data_root, annt_path, start, end, output_dir):
    ...
    for sample_idx, inputs in enumerate(data):
        ...
        meta = inputs["meta"]
        sid = meta["scene_full_id"]+"_"+str(meta["story_id"])
            ...
            for idx in range(start_idx, len(narrative)):
                ...
                if not os.path.exists(gold_img_pth):
                    ...
                    try:
                        ...
                        _, gold_img = get_image(data_root, meta["image_links"][idx], transform)
                        gold_img_pth = os.path.join(gold_save_dir, sid+"_"+str(idx)+".jpg")
