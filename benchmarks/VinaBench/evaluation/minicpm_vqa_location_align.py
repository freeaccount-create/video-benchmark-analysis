    ...
    for exp in experiments[dataset]:
        ...
        for sid, sp in tqdm(enumerate(samples)):
            ...
            for tid, plot in enumerate(sp["narrative"]):
                ...
                msgs = []
                prompt = f'Is this image taken at {art} {loc}? Only answer yes or no.'
                ...
                if res.lower().startswith("yes"):
                    location_align_scores[dataset+"_"+exp].append(1.0)
                else:
                    location_align_scores[dataset+"_"+exp].append(0.0)

final_match = {}
for eid, score_ls in location_align_scores.items():
    final_match[eid] = float(np.mean(score_ls))

with open("./mm_interleaved_minicpm_vqa_location_align_scores.json", "w") as f:
