...
for dataset in ["vwp", "sb20k", "salon_short"]:
    ...
    for exp in experiments[dataset]:
        location_align_scores[dataset+"_"+exp] = []
        ...
        for sid, sp in tqdm(enumerate(samples)):
            ...
            location_info = sp["location"]
    
            for tid, plot in enumerate(sp["narrative"]):
                loc = location_info[tid]

                if any([loc.startswith(letter) for letter in ["a", "e", "i", "o", "u"]]):
                    art = "an"
                else:
                    art = "a"
                ...
                question = DEFAULT_IMAGE_TOKEN + f'\nIs this image taken at {art} {loc}? Only answer yes or no.'
                ...
                location_align_scores[dataset+"_"+exp].append(score)

final_match = {}
for eid, score_ls in location_align_scores.items():
    final_match[eid] = float(np.mean(score_ls))

with open("../evaluation/mm_interleaved_llava_vqa_location_align_scores.json", "w") as f:
