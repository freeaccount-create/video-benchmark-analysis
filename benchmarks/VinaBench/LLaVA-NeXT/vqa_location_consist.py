...
for dataset in ["vwp", "sb20k", "salon_short"]:
    ...
    for exp in experiments[dataset]:
        location_consist_scores[dataset+"_"+exp] = []
        ...
        for sid, sp in tqdm(enumerate(samples)):
            ...
            for loc in loc_set:
                ...
                if len(target_tids) < 2:
                    ...
                    question += f'Are all these images taken at the same {loc}? Only answer yes or no.'
            ...
            if loc_eval_count > 0:
                location_consist_scores[dataset+"_"+exp].append(consist_score/loc_eval_count)

final_scores = {}
for eid, score_ls in location_consist_scores.items():
    final_scores[eid] = float(np.mean(score_ls))

with open("../evaluation/mm_interleaved_llava_vqa_location_consist_scores.json", "w") as f:
